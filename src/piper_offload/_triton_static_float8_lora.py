"""Triton kernels for static per-tensor FP8 LoRA and dense merging."""

# Triton JIT kernel signatures intentionally use untyped pointer parameters
# and upper-case constexpr names.
# ruff: noqa: ANN001, ANN202, N803, PLR0913, PLR0915
...
def _merge_static_float8(
    ...
    *,
    ...
    """Return raw FP8 buffers after one LoRA or dense merge."""
    ...
    compute_dtype = _compute_dtype_id(update.dtype if dense_update else b.dtype)
    ...
    if dense_update and tuple(update.shape) != (rows, cols):
        raise ValueError("Dense update does not match the FP8 weight shape.")
...
def merge_static_float8_dense(
    ...
    *,
    ...
    """Return raw FP8 buffers after one static-per-tensor dense merge."""
    return _merge_static_float8(
        qdata,
        scale,
        update,
