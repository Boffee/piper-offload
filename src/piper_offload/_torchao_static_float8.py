...
layout that Piper Offload preserves and for the generic
...
def _install_rank_agnostic_linear() -> None:
    ...
    if getattr(PrototypeFloat8Tensor, "_piper_offload_linear_patch", False):
        return
    ...
    PrototypeFloat8Tensor._piper_offload_linear_patch = True
...
if TORCHAO_STATIC_FLOAT8_AVAILABLE:
    try:
        ...
        # Keep importing Piper Offload optional when TorchAO refactors its
...
def validate_layout(t: torch.Tensor) -> None:
    ...
    if _LINEAR_PATCH_ERROR is not None:
        raise RuntimeError(
            ...
            f"with this piper-offload version ({_LINEAR_PATCH_ERROR})."
        )
