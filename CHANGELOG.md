...
- Rework host backing ownership so `HostBacking` owns its allocation's full
  ...
  per-copy lease are gone. Asynchronous CUDA copies are issued only while a
  session lease is open and are otherwise made synchronous, so a backing with
  no lease is never being read. Overlap validation between distinct storages
...
- Add `HostBacking.pin()`, `unpin()` and `evict()`. A private file mapping is
  ...
  registering a mapping on a given machine.
...
[Unreleased]: https://github.com/Boffee/piper-offload/compare/v0.9.0rc3...HEAD
[0.9.0rc3]: https://github.com/Boffee/piper-offload/compare/v0.9.0rc2...v0.9.0rc3
[0.9.0rc2]: https://github.com/Boffee/piper-offload/compare/v0.9.0rc1...v0.9.0rc2
